...
public class PrimalUnlicensedCreatures {
   public int maxWins(int initialLevel, int[] grezPower) {
   		int win = 0;
   		int defeat = 0;
   		boolean[] isWin = new boolean[grezPower.length];
   		int myLevel = initialLevel;
   		...
   		for(int i = 0; ; i++){
   			if(i == grezPower.length){ i = 0; }
   			if(myLevel > grezPower[i] && isWin[i] == false){
   				myLevel += grezPower[i] / 2;
   				win++;
   				isWin[i] = true;
   				defeat = 0;
   			}else{ defeat++; }
   			if(defeat == grezPower.length){ break; }
   		}
